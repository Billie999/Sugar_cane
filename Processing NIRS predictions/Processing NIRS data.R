#Assessment 4B-Biljana Simonovikj
...
getwd()
NIRData <- read.table("/Users/Biljana/NIRPred.csv", header = TRUE, sep = ",", quote = "\"'", dec = ".",
                      fill = TRUE, comment.char = "", na.strings =  "?") # reads a file in table format and creates 
#data frame from it named “NIRData”

View(NIRData) # check the data has been loaded correctly
...
#Table 30 Use the following R code to correctly assign the DateTime variable to the POSIXct data type.
...
View(DateTime) # checking if the re-formatting has occured
head(DateTime) # first 6 rows of the column “DateTime

#Table 31 Use transform() with the floor() function applied to ScanID to create a new variable called LabID in the NIRData table. Enter your R code you used to create the new variable,
...
View(NIRData) # check the transformation of the data frame
...
#Table 32 Use a PIPE to sequentially filter the NIR data by filtering out any (a) GH values greater than 3.5, (b)
...
#Save the filtered data to a new data table called NIRData_Filtered.  Enter your R code:
GH <- NIRData$GH
NH <- NIRData$NH
