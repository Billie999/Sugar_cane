...
# character. 
getwd()
Lab_Fibre_Data <- read.table("path", header = TRUE, sep = ",", quote = "\"'", dec = ".")

str(Lab_Fibre_Data)
dim(Lab_Fibre_Data) # check the dimensions of the data frame
head(Lab_Fibre_Data) # first 6 rows
sapply(Lab_Fibre_Data, class) # high-order function returns vector as an output and it is a version of lapply() function
summary(Lab_Fibre_Data)

# Calculate Percentage Fibre Variables: Calculate the fibre percentage of the first set of measurements 
...
# Repeat the above procedure for the second set of measurements (columns 5 to 7, named SampleWeight_2, 
...
# and add it as a new variable to the “Lab_Fibre_Data” data table. 
library(dplyr)
...
# Use the filter() function to remove samples (rows) that contain a missing value in any of the weight measurements. 
...
# Use mutate() to calculate the average fibre and add it as a new variable named “Fibre” 
 #to the Lab_Fibre_Filtered data table. 
...
# Save the resulting data into the Lab_Fibre_Filtered table. 
Thresh.Fibre.min <- 4
Thresh.Fibre.max <- 25
...
Lab_Ash_Data <- read.table("path", header = TRUE, sep = ",", quote = "\"'", dec = ".", na.strings = "0", 
...
# Use a PIPE with the grouped_by() and summarize() functions to produce a data table called Lab_Ash that is grouped by 
...
# LabID and Ash, where LabID now contains unique values (no replicates). 
Ash <- Lab_Ash_Calculated$Ash
...
# Enter your R code you used to import the Pol and Brix data into a data table called “Lab_PB_Data”
getwd()
Lab_PB_Data <- read.table("path", header = TRUE, sep = ",", quote = "\"'", dec = ".")
...
# Use ggplot() to visually display a scatter (i.e. point) plot of variables Brix and Pol. Colour code the points according 
...
# Use a PIPE to sequentially filter out undesirable rows from Lab_PB_Data, and then select only a subset of its variables, as follows: 
...
# Use the following R code to join the Fibre and Ash tables together
...
# Use transform() to transform the Fibre measurements from the Lab_Fibre table using the provided z_stand() function.  
...
# Use a pipe with two subsequent transform() operations to transform the Ash measurements from the Lab_Ash table, first using log10(), 
...
# Use the following R code to create a variable which can be subsequently used for stratified sub-sampling: 
Lab_PB$Bbin <- cut(Lab_PB$Brix, 40, labels = FALSE)
Bbin <- Lab_PB$Bbin
length(Bbin)
head(Bbin)
                  
# Use the following R code to re-cast the Bbin variable as ordinal, so it can be used for stratified sub-sampling: 
...
# Use the group_by() function then the sample_n() function to perform stratified sampling on the 
...
# Use transform() with rescale_01() to rescale the Brix measurements in Lab_B_Stratified_Balanced. 
